...
function PendingCard({ r, reviewerId, eggName }: { r: EggReturn; reviewerId: string; eggName: Map<string, string> }) {
            ...
            return (
              <div key={it.id} className="rounded-xl border border-gray-200 dark:border-gray-700 p-3">
                <p className="text-sm text-gray-800 dark:text-gray-200">
                  {eggName.get(it.egg_type_id) ?? 'Producto'} — declaró <strong>{it.qty}</strong> (vence {it.expiry_date})
                </p>
                <div className="flex flex-wrap items-end gap-3 mt-2">
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-gray-500 dark:text-gray-400">Le devolvés</label>
...
function ReviewedCard({ r, eggName }: { r: EggReturn; eggName: Map<string, string> }) {
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5 space-y-1">
      ...
      {r.items.map((it) => (
        <p key={it.id} className="text-sm text-gray-700 dark:text-gray-300">
          📦 {eggName.get(it.egg_type_id) ?? 'Producto'}: <strong>{it.qty}</strong> (vence {it.expiry_date})
          {r.status === 'aprobada' && it.returned_qty != null
            ? ` · devueltos ${it.returned_qty}${it.returned_expiry_date ? ` (vence ${it.returned_expiry_date})` : ''}`
            : ''}
        </p>
      ))}
    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {rows.map((r) =>
            r.status === 'pendiente'
              ? <PendingCard key={r.id} r={r} reviewerId={profile?.id ?? ''} eggName={eggName} />
              : <ReviewedCard key={r.id} r={r} eggName={eggName} />,
          )}
        </div>
      )}
    </div>
